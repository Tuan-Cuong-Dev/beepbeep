// Phiên bản hoàn thiện sau khi migrate
...
/** Lấy toạ độ hiển thị theo chuẩn mới: location.geo → '-' */
const getCoordsText = (p: TechnicianPartner) => {
  const gp = (p as any)?.location?.geo;
      ...
      <div className="block md:hidden space-y-4">
        {paginatedPartners.map((partner) => (
          <div key={partner.id} className="border rounded-xl p-4 shadow-sm space-y-2 text-sm bg-white">
            ...
            <div>{t('technician_partner_table.col.coordinates')}: {getCoordsText(partner)}</div>
