...
export type PrivateProviderFormData = {
  name: string;
  email: string;
  phone: string;
...
interface Props {
  ...
  onSave: (data: PrivateProviderFormData) => Promise<void>;
  onCancel: () => void;
}

export default function PrivateProviderForm({ editingCompany, onSave, onCancel }: Props) {
  const [form, setForm] = useState<PrivateProviderFormData>({
    name: '',
    email: '',
    phone: '',
    displayAddress: '',
    ...
    <div className="space-y-5">
      ...
      <div className="flex justify-end gap-4 pt-4">
        <Button onClick={handleSubmit} className="bg-[#00d289] text-white hover:bg-[#00b67a]">
          {editingCompany ? 'Update' : 'Add'} Provider
        </Button>
        {editingCompany && (
