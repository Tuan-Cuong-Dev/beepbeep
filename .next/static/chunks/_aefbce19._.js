...
function MyBusinessPage() {
    ...
    (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$index$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["useEffect"])({
        "MyBusinessPage.useEffect": ()=>{
            const fetchBusinesses = {
                "MyBusinessPage.useEffect.fetchBusinesses": async ()=>{
                    if (!user?.uid) return;
                    ...
                    const loaded = [];
                    rentalSnap.forEach({
                        "MyBusinessPage.useEffect.fetchBusinesses": (doc)=>{
                            loaded.push({
                                id: doc.id,
                                name: doc.data().name || 'Rental Company',
                                type: 'rental_company'
                            });
                        }
                    }["MyBusinessPage.useEffect.fetchBusinesses"]);
                    agentSnap.forEach({
                        "MyBusinessPage.useEffect.fetchBusinesses": (doc)=>{
                            loaded.push({
                                id: doc.id,
                                name: doc.data().name || 'Agent',
                                type: 'agent'
                            });
                        }
                    }["MyBusinessPage.useEffect.fetchBusinesses"]);
    ...
    const handleSelectBusiness = (business)=>{
        if (business.type === 'rental_company') {
            router.push(`/my-business/rental-company/${business.id}`);
        } else if (business.type === 'private_provider') {
            router.push(`/my-business/private-provider/${business.id}`);
        } else if (business.type === 'agent') {
            router.push(`/my-business/agent/${business.id}`);
        }
    };
    ...
    if (businesses.length === 1) {
        // Nếu chỉ có 1 business -> tự động điều hướng luôn
...
_s(MyBusinessPage, "5BGB7jPY9K9mNxKysJ3EML/Om4A=", false, function() {
    return [
...
//# sourceMappingURL=_aefbce19._.js.map