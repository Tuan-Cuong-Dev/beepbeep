/**
 * Email qua SendGrid hoặc SES.
 * Bạn có thể bật 1 trong 2 bằng ENV:
 * - SENDGRID_API_KEY   + SENDGRID_FROM
 * - hoặc: SES_REGION, AWS_ACCESS_KEY_ID, AWS_SECRET_ACCESS_KEY, SES_FROM
 *
 * Mặc định ưu tiên SendGrid nếu có key.
 */
...
export async function sendEmail(
  ...
  if (!target.to) return { provider: 'email', status: 'skipped', errorMessage: 'Missing email' };
