...
/* ========== Helpers ========== */
function renderTemplate(tplStr = "", data: any): string {
  return tplStr.replace(/\{\{\s*([\w.]+)\s*\}\}/g, (_: any, k: string) => {
    const v = k.split(".").reduce((o: any, kk: string) => o?.[kk], data);
    return (v ?? "").toString();
  });
}
...
async function callWorker(path: string, body: unknown) {
  if (!FUNCTIONS_BASE) throw new Error("FUNCTIONS_BASE_URL is not set");
  ...
  .onRun(async (): Promise<void> => {
    ...
    for (const doc of snap.docs) {
      try {
        ...
        const pref = prefSnap.data() as any;
