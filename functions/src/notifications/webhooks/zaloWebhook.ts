    ...
    try {
      ...
      if (q.empty) {
        functions.logger.warn(
          'Zalo webhook: delivery not found for message_id',
          messageId
        );
        res.json({ ok: true });
        return;
      ...
      const mapStatus = (s?: string) => {
        ...
        if (x.includes('seen') || x.includes('read')) return 'read' as const;
        if (x.includes('deliver')) return 'delivered' as const;
        if (x.includes('fail') || x.includes('error')) return 'failed' as const;
