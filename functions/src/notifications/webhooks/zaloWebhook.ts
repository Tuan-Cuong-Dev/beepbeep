...
function verifySignature(req: functions.https.Request, appSecret?: string) {
  if (!appSecret) return true; // chưa bật secret → bỏ qua
  const sig = (req.get('x-zalo-signature') || '').trim();
  if (!sig) return false;

  const raw = (req as any).rawBody?.length
  ...
  .https.onRequest(async (req, res): Promise<void> => {
    try {
      ...
      const zaloUserId: string | undefined =
        body?.sender?.id || body?.follower?.id || body?.user_id || body?.user?.id;
      ...
      if (event === 'user_send_text') {
        ...
        if (zaloUserId) {
          await db.doc(`zalo_oa/users/${zaloUserId}`).set(
            { lastSeenAt: FieldValue.serverTimestamp(), lastMessage: text ?? null },
            { merge: true }
          );
        }
        ...
        const uid = linkDoc?.exists ? (linkDoc.data() as any)?.uid : undefined;
        ...
        // Ghi map zalo ↔ uid
        await db.doc(`zalo_oa/users/${zaloUserId}`).set(
          { linkedUid: uid, linkedAt: FieldValue.serverTimestamp(), followed: true },
          { merge: true }
        );
        ...
        await db.doc(`linkCodes/${code}`).set(
          { status: 'used', usedAt: FieldValue.serverTimestamp(), usedBy: zaloUserId },
          { merge: true }
        );
      ...
      if (event === 'user_follow') {
        if (zaloUserId) {
          await db.doc(`zalo_oa/users/${zaloUserId}`).set(
            { followed: true, lastSeenAt: FieldValue.serverTimestamp() },
            { merge: true }
          );
        }
      ...
      if (event === 'user_unfollow') {
        if (zaloUserId) {
          await db.doc(`zalo_oa/users/${zaloUserId}`).set(
            { followed: false, unfollowedAt: FieldValue.serverTimestamp() },
            { merge: true }
          );
        }
      ...
      const messageId =
        body?.message?.msg_id || body?.message_id || body?.data?.message_id;
      ...
      if (messageId) {
        ...
        if (!q.empty) {
          ...
          const now = Date.now();
          const mapped =
            status.includes('seen') || status.includes('read')
              ? 'read'
