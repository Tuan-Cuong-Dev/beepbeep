...
type LinkBody =
  | { action: "link"; zaloUserId: string; code: string; raw?: any }
  | { action: "follow"; zaloUserId: string; raw?: any }
  | { action: "unfollow"; zaloUserId: string; raw?: any };
...
export const zaloIngest = functions
  ...
  .https.onRequest(async (req, res) => {
    try {
      ...
      if ((req.header("x-internal-secret") || "") !== INTERNAL) { res.status(401).send("Unauthorized"); return; }

      const body = req.body as LinkBody;
      if (!body?.action) { res.status(400).json({ ok: false, error: "BAD_ACTION" }); return; }
      ...
      if (body.action === "follow") {
        ...
        res.json({ ok: true, action: "followed" }); return;
      }
      ...
      if (body.action === "unfollow") {
        ...
        res.json({ ok: true, action: "unfollowed" }); return;
      }
      ...
      if (body.action === "link") {
        ...
        if (!zaloUserId || !code) {
          res.status(400).json({ ok: false, error: "MISSING_PARAMS" }); return;
        }

        // Lấy doc theo docId = code (chuẩn mình đã tạo)
        const codeRef = db.collection(COLL_LINK_CODES).doc(code);
        const codeSnap = await codeRef.get();
        if (!codeSnap.exists) { res.status(404).json({ ok: false, error: "CODE_NOT_FOUND" }); return; }
        const data = codeSnap.data() as any;

        if (data.used) { res.status(409).json({ ok: false, error: "CODE_USED" }); return; }
        const expiresAtMs: number = Number(data.expiresAtMs ?? data.expiresAt ?? 0);
        if (!expiresAtMs || Date.now() > expiresAtMs) {
          res.status(410).json({ ok: false, error: "CODE_EXPIRED" }); return;
        }
        const uid: string | undefined = data.uid;
        if (!uid) { res.status(500).json({ ok: false, error: "CODE_NO_UID" }); return; }
        ...
        res.json({ ok: true, uid, zaloUserId, linked: true }); return;
      }
