...
// —— Kiểu payload phân biệt theo action (discriminated union)
type FollowPayload   = { action: "follow";   zaloUserId: string; raw?: any };
type UnfollowPayload = { action: "unfollow"; zaloUserId: string; raw?: any };
type LinkPayload     = { action: "link";     zaloUserId: string; code: string; raw?: any };
type Payload = FollowPayload | UnfollowPayload | LinkPayload;

// Type guard cho nhánh link
function isLinkPayload(p: Payload): p is LinkPayload {
...
export const zaloIngest = functions
  ...
  .https.onRequest(async (req, res): Promise<void> => {
    try {
      ...
      if (!expected || got !== expected) { res.status(401).send("Unauthorized"); return; }
      ...
      if (isLinkPayload(body)) {
        ...
        if (!code) { res.status(400).json({ ok:false, error:"MISSING_CODE" }); return; }

        const codeRef = db.doc(`zalo_link_codes/${code}`);
        const snap = await codeRef.get();
        if (!snap.exists) { res.status(404).json({ ok:false, error:"CODE_NOT_FOUND" }); return; }

        const data = snap.data() as { uid?: string; expiresAt?: number; used?: boolean };
        const uid = data?.uid;
        if (!uid) { res.status(400).json({ ok:false, error:"CODE_NO_UID" }); return; }
        if (data.used) { res.status(400).json({ ok:false, error:"CODE_USED" }); return; }
        if (data.expiresAt && data.expiresAt < now) { res.status(400).json({ ok:false, error:"CODE_EXPIRED" }); return; }
        ...
        res.json({ ok: true, uid, zaloUserId }); return;
      }
